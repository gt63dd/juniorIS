...
Prior work shows that navigation and timing signals in web logs can be used to detect evasive bots, and that combining multiple evidence sources can improve robustness. Iliou et al.\ demonstrate that modeling sessions from web logs can be effective, and that adding human-interaction evidence (mouse behavior) raises attacker cost and improves detection under an “advanced bot” threat model \cite{10.1145/3447815}. However, bot detection is not a static problem. Iliou et al.\ also show an adversarial dynamic where reinforcement-learning bots learn navigation behaviors that evade detectors trained on web-log features, and can re-adapt after the detector is retrained \cite{10.1145/3538969.3538994}. This motivates designing a risk score that can be stress-tested against adaptive policies rather than only simple scripted bots. Finally, risk scoring is only valuable if it connects to mitigation decisions. Gangwal et al.\ propose a multi-barrier CAPTCHA mechanism that balances usability and security, suggesting a pathway for escalation when a session appears suspicious \cite{10.1145/3672608.3707741}. Together, these sources motivate a layered approach: (1) score sessions using navigation patterns, (2) escalate friction for high-risk traffic, and (3) evaluate robustness under evasion.
...
\begin{enumerate}
	...
	\item Stress-test robustness by introducing adaptive behavior inspired by reinforcement-learning evasion results \cite{10.1145/3538969.3538994}.
\end{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Robustness:} performance under stealth bots and adaptive behavior inspired by RL evasion \cite{10.1145/3538969.3538994}.
\end{itemize}
...
\begin{tikzpicture}[node distance=14mm, every node/.style={font=\small}]
	ikzset{box/.style={draw, rounded corners, align=center, minimum width=3.7cm, minimum height=0.9cm}}
...
\begin{enumerate}
	\item Implement product page endpoint and static product inventory model.(1)
	\item Implement cart endpoints (add, remove, update quantity/size).(1)
	\item Implement checkout endpoint and simulated ``purchase'' action that decrements inventory.(1)
	\item Create session management (session ID cookies) for all clients.(4)
	\item Implement structured event logging to SQLite (timestamp, session ID, endpoint, status, metadata).(1.5)
	\item Implement feature extraction for navigation patterns from event logs (timing, path, burstiness).(2)
	\item Implement risk scoring function returning a bot-likelihood score in $[0,1]$.(3)
	\item Implement mitigation policy gates at add-to-cart and checkout based on risk score.(1)
	\item Build bot simulator scripts: fast-path bot, burst/polling bot, stealth bot.(13)
	\item Build a human-session simulator with realistic timing variance and navigation noise.(1)
	\item Implement experiment runner to launch mixed populations and run repeated drop trials.(2)
	\item Implement evaluation metrics and produce summary tables/plots (capture rate, false positives, friction).(3)
